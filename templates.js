...
// server-> 
// client -> maintains state with -> server
// client -> user -> has-token from -> server
// server -> tracks -> client state 


// client -> interaction -> server -> event trigger 


// client functionality -> connect clients -> via video and chat 

// client to client connection -> via server -> connected peer to peer; 
  ...
  form_values(fields, tag_properties) {
    ...
    while (client_form.length) {
      ...
      for (const [key, value] of Object.entries(input)) {
        ...
        value.constructor === Object ? client_form.push([temp, value]) : this.section_generator(label.join(""), value, tag_properties);
...
if(tag_properties[route]){
    ...
    const display_value = trans_form.charAt(0).toUpperCase() + trans_form.slice(1, route.length)

    const tag = (tag_properties[route] || {});

    this.section.push(`<div class="input-row">
    <label class="form-label" for="${route}">${display_value}:</label>
    <input class="input-group-text ${tag_properties.css_state || ""}" id=${route} prop="${route}" type="${tag.type || "text"}" placeholder="${value || tag.example || display_value}"  
   ${tag.pattern ? 'pattern=' + tag.pattern : ''} value="${value || ""}" title="${display_value}" required autocomplete="off" ${tag.disabled ? "disabled" : ""} />
    </div>`);
}
  }
  signup() {
    return this.form_values({
      ...
      "balance": { type: "number", disabled: true }, "password": { type: "password", disabled: false, title: "Your password" },
...
export async function html_snippets() {
  ...
  let paths = await fileSystem.readdirAsync(root_dir + "/client/html")
  paths = paths.map(file => root_dir + "/client/html/" + file)
  for (const dir of paths) {

    const is_file = await fileSystem.statAsync(dir).catch(console.error)

    if (await is_file.isDirectory()) {
      (await fileSystem.readdirAsync(dir).catch(console.error)).map(file => paths.push(dir + "/" + file))
    }
  ...
  while (paths.length) {
    let file = paths.pop();
    file.includes(".") && Files.set(file.split("/").pop().split(".").shift(),
...
export default async function tempting_service(user, template_id) {
  const { user_interface } = await responsibility();
