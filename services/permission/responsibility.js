...
export async function authorization_json(add_fields = "admin.coordinator") {
  const auth_object = JSON.parse(
    readFileAsync(join(this_dir, "authentication_routes.json"), {
      encoding: "utf8",
    })
  );
  ...
  writeFileAsync(
    join(this_dir, "authentication_routes.json", { encoding: "utf8" })
  );
}
...
export function get_user_interface(table) {
  ...
  return function (permission_chain) {
    permission_chain =
      Boolean(permission_chain) && permission_chain.responsibility;
    ...
    const is_user_seated = Boolean(
      permission_chain &&
        permission_chain
          .split(".")
          .every((prop) => [(temp_table = temp_table[prop]), temp_table][1])
    );
    ...
    responsibility_table = new Map(
      Object.entries(responsibility_table).filter(
        (prop) => prop[1].constructor === String
      )
    );
